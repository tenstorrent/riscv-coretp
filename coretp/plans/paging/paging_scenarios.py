...
# Riescue-D does not support recursive PTEs
# @paging_scenario
...
def SID_PBVMS_009_superpage_alignment_fault():
    ...
    read_leaf_2m = ReadPTE(memory=mem_2m, level=PteLevel.LEAF)
    li_misalign = LoadImmediateStep(imm=0x400)
    or_misalign = Arithmetic(op="or", src1=read_leaf_2m, src2=li_misalign)
    write_leaf_2m = WritePTE(memory=mem_2m, level=PteLevel.LEAF, src=or_misalign)
...
# Disable temporarily - need a way to track excp deleg since this test needs machine excp delegation for Load Fault
# @paging_scenario
...
#     save_pte_step = Arithmetic(op="mv", src1=read_pte)
...
#     comment_3 = Comment(comment="Return PTE value")
#     write_pte_2 = WritePTE(memory=mem_nl, level=1, src=save_pte_step)

#     return TestScenario.from_steps(
#         id="10",
#         name="SID_PBVMS_010_pte_reserved_values_fault",
#         description="PTEs with reserved values for non-leaf, leaf PTEs",
#         env=TestEnvCfg(paging_modes=[PagingMode.SV39, PagingMode.SV48, PagingMode.SV57], deleg_excp_to=[PrivilegeMode.M]),
#         steps=[
#             comment_1,
#             mem,
#             assert_load_fault,
...
def SID_PBVMS_015_global_bit_honoured():
    ...
    mv_out = Arithmetic(op="mv", src1=original_satp)
    ...
    restore_satp = CsrWrite(csr_name="satp", value=mv_out)
...
def SID_PBVMS_016_non_global_bit_honoured():
    ...
    mem = Memory(size=0x1000, page_size=PageSize.SIZE_4K, flags=PageFlags.VALID | PageFlags.READ | PageFlags.WRITE, exclude_flags=PageFlags.GLOBAL, modify=True)
    ...
    write_leaf = WritePTE(memory=mem, level=PteLevel.LEAF, src=clear_read)
    ...
    comment_7 = Comment(comment="Modify leaf PTE to re-add READ bit")
    read_leaf_2 = ReadPTE(memory=mem, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read = Arithmetic(op="or", src1=read_leaf_2, src2=li_add_r)
    write_leaf_ok = WritePTE(memory=mem, level=PteLevel.LEAF, src=add_read)
...
def SID_PBVMS_018_rsw_fields_writable():
    ...
    write_leaf = WritePTE(memory=mem, level=PteLevel.LEAF, src=or_rsw)
...
def SID_PBVMS_024_satp_mode_transitions():
    ...
    li_60_63 = LoadImmediateStep(imm=0xF000000000000000)
    clear_top = CsrWrite(csr_name="satp", clear_mask=li_60_63, direct_write=True)

    comment_3 = Comment(comment="Transition: Bare -> SV39")
    li_sv39 = LoadImmediateStep(imm=0x8000000000000000)
    set_sv39 = CsrWrite(csr_name="satp", set_mask=li_sv39, direct_write=True)
    read_sv39 = CsrRead(csr_name="satp", direct_read=True)

    comment_4 = Comment(comment="Transition: SV39 -> SV48")
    li_sv48 = LoadImmediateStep(imm=0x9000000000000000)
    clear_top_2 = CsrWrite(csr_name="satp", clear_mask=li_60_63, direct_write=True)
    set_sv48 = CsrWrite(csr_name="satp", set_mask=li_sv48, direct_write=True)
    read_sv48 = CsrRead(csr_name="satp", direct_read=True)

    comment_5 = Comment(comment="Restore original satp")
    restore_satp = CsrWrite(csr_name="satp", value=mv_out, direct_write=True)
...
def SID_PBVMS_026_tlb_asid_match():
    ...
    read_satp = CsrRead(csr_name="satp", direct_read=True)
    set_asid_a1 = CsrWrite(csr_name="satp", set_mask=0x1 << 44, direct_write=True)
    load_a1 = Load(memory=mem)
    ...
    li_clear_r = LoadImmediateStep(imm=~0x2)
    clear_read = Arithmetic(op="and", src1=read_leaf, src2=li_clear_r)
    write_leaf_fault = WritePTE(memory=mem, level=PteLevel.LEAF, src=clear_read)

    comment_4 = Comment(comment="Set ASID=A2 and access -> should NOT hit TLB (different ASID)")
    set_asid_a2 = CsrWrite(csr_name="satp", set_mask=0x2 << 44, direct_write=True)
    sfence = Arithmetic(op="sfence.vma")
    assert_load_fault = AssertException(cause=ExceptionCause.LOAD_PAGE_FAULT, code=[Load(memory=mem)])

    comment_5 = Comment(comment="Modify PTE to re-add READ bit ")
    read_leaf_2 = ReadPTE(memory=mem, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read = Arithmetic(op="or", src1=read_leaf_2, src2=li_add_r)
    write_leaf_ok = WritePTE(memory=mem, level=PteLevel.LEAF, src=add_read)
...
def SID_PBVMS_028_sfence_vma_opcode_coverage():
    ...
    mem1 = Memory(size=0x1000, page_size=PageSize.SIZE_4K, flags=PageFlags.VALID | PageFlags.READ | PageFlags.WRITE)
    load_immediate = LoadImmediateStep(imm=mem1)
    load_immediate_2 = LoadImmediateStep(imm=0x1)  # random value
    ...
    sfence_va = Arithmetic(op="sfence.vma", src1=load_immediate, src2=0)
    comment_3 = Comment(comment="sfence.vma x0, rs2 (ASID-based invalidation)")
    sfence_asid = Arithmetic(op="sfence.vma", src1=0, src2=load_immediate_2)

    comment_4 = Comment(comment="sfence.vma rs1, rs2 (VA+ASID-based invalidation)")
    sfence_va_asid = Arithmetic(op="sfence.vma", src1=load_immediate, src2=load_immediate_2)
...
def SID_PBVMS_030_sfence_vma_ordering_sync():
    ...
    # RETURN PTE
    comment_5 = Comment(comment="Return PTE")
    read_leaf_2 = ReadPTE(memory=mem, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read = Arithmetic(op="or", src1=read_leaf_2, src2=li_add_r)
    write_leaf_ok = WritePTE(memory=mem, level=PteLevel.LEAF, src=add_read)
...
def SID_PBVMS_031_sfence_vma_invalidate_all():
    ...
    read_leaf_g = ReadPTE(memory=mem_global, level=PteLevel.LEAF)
    li_clear_r = LoadImmediateStep(imm=~0x2)
    clear_read_g = Arithmetic(op="and", src1=read_leaf_g, src2=li_clear_r)
    write_leaf_g = WritePTE(memory=mem_global, level=PteLevel.LEAF, src=clear_read_g)
    read_leaf_ng = ReadPTE(memory=mem_nonglobal, level=PteLevel.LEAF)
    clear_read_ng = Arithmetic(op="and", src1=read_leaf_ng, src2=li_clear_r)
    write_leaf_ng = WritePTE(memory=mem_nonglobal, level=PteLevel.LEAF, src=clear_read_ng)
    ...
    # RETURN PTE
    comment_7 = Comment(comment="Return PTE")
    read_leaf_g_2 = ReadPTE(memory=mem_global, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read_g = Arithmetic(op="or", src1=read_leaf_g_2, src2=li_add_r)
    write_leaf_g_ok = WritePTE(memory=mem_global, level=PteLevel.LEAF, src=add_read_g)
    read_leaf_ng_2 = ReadPTE(memory=mem_nonglobal, level=PteLevel.LEAF)
    add_read_ng = Arithmetic(op="or", src1=read_leaf_ng_2, src2=li_add_r)
    write_leaf_ng_ok = WritePTE(memory=mem_nonglobal, level=PteLevel.LEAF, src=add_read_ng)
    return TestScenario.from_steps(
        id="31",
...
def SID_PBVMS_032_sfence_vma_invalidate_asid():
    ...
    clear_asid1 = CsrWrite(csr_name="satp", clear_mask=0xFFFF00000000000)
    set_asid1 = CsrWrite(csr_name="satp", set_mask=0x1 << 44)
    load_a1 = Load(memory=mem)
    ...
    clear_asid2 = CsrWrite(csr_name="satp", clear_mask=0xFFFF00000000000)
    set_asid2 = CsrWrite(csr_name="satp", set_mask=0x2 << 44)
    asid = LoadImmediateStep(imm=0x1)
    sfence_asid = Arithmetic(op="sfence.vma", src1=0, src2=asid)

    comment_5 = Comment(comment="Set ASID=A1, access should fault")
    clear_asid1_again = CsrWrite(csr_name="satp", clear_mask=0xFFFF00000000000)
    ...
    # RETURN PTE
    comment_6 = Comment(comment="Return PTE")
    read_leaf_2 = ReadPTE(memory=mem, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read = Arithmetic(op="or", src1=read_leaf_2, src2=li_add_r)
    write_leaf_ok = WritePTE(memory=mem, level=PteLevel.LEAF, src=add_read)
...
def SID_PBVMS_033_sfence_vma_invalidate_va():
    ...
    load_immediate = LoadImmediateStep(imm=mem_global)
    load_immediate_2 = LoadImmediateStep(imm=mem_nonglobal)  # random value
    sfence_va = Arithmetic(op="sfence.vma", src1=load_immediate, src2=0)
    sfence_va_2 = Arithmetic(op="sfence.vma", src1=load_immediate_2, src2=0)

    comment_5 = Comment(comment="Access MUTs should fault after VA-based invalidation")
    assert_fault_g = AssertException(cause=ExceptionCause.LOAD_PAGE_FAULT, code=[Load(memory=mem_global)])
    assert_fault_ng = AssertException(cause=ExceptionCause.LOAD_PAGE_FAULT, code=[Load(memory=mem_nonglobal)])

    comment_6 = Comment(comment="Modify leaf PTE to re-add READ bit")
    read_leaf_g_2 = ReadPTE(memory=mem_global, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read_g = Arithmetic(op="or", src1=read_leaf_g_2, src2=li_add_r)
    write_leaf_g_ok = WritePTE(memory=mem_global, level=PteLevel.LEAF, src=add_read_g)
...
def SID_PBVMS_034_sfence_vma_invalidate_va_asid():
    ...
    asid = LoadImmediateStep(imm=0x1)
    sfence_va_asid = Arithmetic(op="sfence.vma", src1=va, src2=asid)
...
def SID_PBVMS_035_sfence_vma_various_page_sizes():
    ...
    read_leaf_4k = ReadPTE(memory=mem_4k, level=PteLevel.LEAF)
    li_clear_r = LoadImmediateStep(imm=~0x2)
    clear_read_4k = Arithmetic(op="and", src1=read_leaf_4k, src2=li_clear_r)
    write_leaf_4k = WritePTE(memory=mem_4k, level=PteLevel.LEAF, src=clear_read_4k)
    ...
    li_clear_r_2 = LoadImmediateStep(imm=~0x2)
    read_leaf_2m = ReadPTE(memory=mem_2m, level=PteLevel.LEAF)
    clear_read_2m = Arithmetic(op="and", src1=read_leaf_2m, src2=li_clear_r_2)
    write_leaf_2m = WritePTE(memory=mem_2m, level=PteLevel.LEAF, src=clear_read_2m)
    ...
    # RETURN PTE
    comment_3 = Comment(comment="Return PTE")
    read_leaf_2 = ReadPTE(memory=mem_2m, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read = Arithmetic(op="or", src1=read_leaf_2, src2=li_add_r)
    write_leaf_ok = WritePTE(memory=mem_2m, level=PteLevel.LEAF, src=add_read)
    read_leaf_3 = ReadPTE(memory=mem_4k, level=PteLevel.LEAF)
    li_add_r_2 = LoadImmediateStep(imm=0x2)
    add_read_2 = Arithmetic(op="or", src1=read_leaf_3, src2=li_add_r_2)
    write_leaf_2_ok = WritePTE(memory=mem_4k, level=PteLevel.LEAF, src=add_read_2)
...
def SID_PBVMS_036_sfence_vma_satp_bare():
    ...
    return TestScenario.from_steps(
        ...
        env=TestEnvCfg(priv_modes=[PrivilegeMode.S], paging_modes=[PagingMode.BARE]),
        steps=[
            comment_1,
            li_bare,
            write_satp_bare,
...
# non canonical VA still has whisper TLB inval
# @paging_scenario
...
#     sfence_nc = Arithmetic(op="sfence.vma", src1=li_non_canonical, src2=0)
...
#             comment_4, load_2,
#             comment_5, read_leaf_2, li_add_r, add_read, write_leaf_ok,
#         ],
#     )
...
def SID_PBVMS_039_non_leaf_not_invalidated_va_based():
    ...
    va = LoadImmediateStep(imm=mem)
    sfence_va = Arithmetic(op="sfence.vma", src1=va, src2=0)
...
def SID_PBVMS_041_partial_page_invalidation():
    ...
    comment_2 = Comment(comment="Modify PTE to introduce fault (clear Read bit)")
    read_leaf = ReadPTE(memory=mem_2m, level=PteLevel.LEAF)
    li_clear_r = LoadImmediateStep(imm=~0x2)
    clear_read = Arithmetic(op="and", src1=read_leaf, src2=li_clear_r)
    write_leaf = WritePTE(memory=mem_2m, level=PteLevel.LEAF, src=clear_read)
    ...
    comment_5 = Comment(comment="Modify leaf PTE to re-add READ bit")
    read_leaf_2 = ReadPTE(memory=mem_2m, level=PteLevel.LEAF)
    li_add_r = LoadImmediateStep(imm=0x2)
    add_read = Arithmetic(op="or", src1=read_leaf_2, src2=li_add_r)
    write_leaf_ok = WritePTE(memory=mem_2m, level=PteLevel.LEAF, src=add_read)
