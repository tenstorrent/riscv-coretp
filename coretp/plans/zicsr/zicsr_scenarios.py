...
def SID_ZICSR_01():
    ...
    """
    comment = Comment(comment="CSRRW: Atomic swap with 0xFFFFF, rd is non-zero")

    csr_access = CsrAccess(op="csrrw", csr_name=None, direct_access=True, src1=0xFFFFF, target_is_x0=False)
...
def SID_ZICSR_02():
    ...
    Ensure that CSR matches privilege mode. Do direct accesses only
    """
    comment = Comment(comment="CSRRW: Atomic swap with 0xFFFFF, rd is x0 (no read)")

    csr_access = CsrAccess(op="csrrw", csr_name=None, direct_access=True, src1=0xFFFFF, target_is_x0=True)
...
def SID_ZICSR_04():
    ...
    Ensure that CSR matches privilege mode. Do direct accesses only
    """
    comment = Comment(comment="CSRRS: Set all bits using mask 0xFFFFF")

    csr_access = CsrAccess(op="csrrs", csr_name=None, direct_access=True, src1=0xFFFFF, target_is_x0=False)
...
def SID_ZICSR_06():
    ...
    Ensure that CSR matches privilege mode. Do direct accesses only
    """
    comment = Comment(comment="CSRRC: Clear all bits using mask 0xFFFFF")

    csr_access = CsrAccess(op="csrrc", csr_name=None, direct_access=True, src1=0xFFFFF, target_is_x0=False)
