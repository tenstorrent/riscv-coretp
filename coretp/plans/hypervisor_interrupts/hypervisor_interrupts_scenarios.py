...
# so hvien[13]/hvip[13] gate vsie[13]/vsip[13] for the VS-level LCOFI.
LCOFI = 1 << 13  # bit 13 in hvien/hvip/sip/vsie/vsip

# VS-mode pending/enable bit positions as seen in vsip/vsie (S-shifted layout).
VSEIE_POS = 1 << 9  # vsie[9] / vsip[9] = VS external
VSTIE_POS = 1 << 5  # vsie[5] / vsip[5] = VS timer
VSSIE_POS = 1 << 1  # vsie[1] / vsip[1] = VS software


# hstatus.VGEIN field (bits 17:12) selects guest external interrupt file.
def hstatus_vgein(n: int) -> int:
    """hstatus.VGEIN field value (bits 17:12)."""
    return (n & 0x3F) << 12
